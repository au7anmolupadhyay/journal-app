...
import net.au7.journalApp.service.SentimentAnalysisService;
    ...
    public void fetchUserAndSendSAMail(){
        ...
        for(User user: users){
            ...
            List<String> filteredEntries = journalEntries.stream().filter(x->x.getDate()
                    .isAfter(LocalDateTime.now().minus(7, ChronoUnit.DAYS))).map(x->x.getContent()).collect(Collectors.toList());
